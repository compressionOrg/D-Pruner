    ...
    with torch.no_grad():
        for name, param in parameters:
            if "self_attn.q_proj." in name:
                ...
                all_values = masks[layer].flatten().cpu()
                threshold = np.percentile(all_values.numpy(), 52)
                modify_tensor_values(masks[layer], threshold)
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python save_model.py LLaMA2_HF_LOCATION IMPORTANCE_LOCATION OUTPUT_LOCATION")
        sys.exit(1)
