...
class LOMOTrainer:
    def __init__(
        ...
        self.lr = 0
        self.sparsity = 0
        self.target_sparsity = 50
        warmup_increment, regular_increment = int(self.num_steps_per_epoch * self.training_args.num_train_epochs * self.training_args.warmup)//5, int(self.num_steps_per_epoch * self.training_args.num_train_epochs * 0.15)
        self.warmup_sparse = [warmup_increment*(j+1) for j in range(5)]


        self.global_steps_sparse = [regular_increment*(j+1) for j in range(6)]

        self.optimizer = LOMO(model, training_args.llama_type, training_args.general_importance_dir, self.lr, training_args.clip_grad_norm, training_args.clip_grad_value)
        ...
        for epoch in range(self.training_args.num_train_epochs):
            ...
            with tqdm.tqdm(self.train_dataloader, disable=not self.allow_print) as tqb:
                for step, batch in enumerate(tqb, start=1):
                    ...
                    # print("regularization:", lamda * regularization)
                    ...
                    if step >= len(tqb):
                        self.optimizer.fused_backward(loss, self.lr, True, epoch)
                    else:
            ...
            if self.training_args.save_strategy == 'epoch':
                ...
                if training_args.llama_type == '7b':
                    increment = 32
                elif training_args.llama_type == '13b':
                    increment = 40
                elif training_args.llama_type == '70b':
                    increment = 80

                e = '/Q/'
                for l in range(increment):
                    save_path = '../importance/' + str(epoch) + e
                    if not os.path.exists(save_path):
                        os.makedirs(save_path, exist_ok=True)
                    torch.save(self.optimizer.importance[l], save_path + str(l) + '.pt')
                ...
                for l in range(increment):
                    ...
                    if not os.path.exists(save_path):
                        os.makedirs(save_path, exist_ok=True)
                    torch.save(self.optimizer.importance[increment+l], save_path + str(l) + '.pt')
                ...
                for l in range(increment):
                    ...
                    if not os.path.exists(save_path):
                        os.makedirs(save_path, exist_ok=True)
                    torch.save(self.optimizer.importance[increment*2+l], save_path + str(l) + '.pt')
                ...
                for l in range(increment):
                    ...
                    if not os.path.exists(save_path):
                        os.makedirs(save_path, exist_ok=True)
                    torch.save(self.optimizer.importance[increment*3+l], save_path + str(l) + '.pt')
                ...
                for l in range(increment):
                    ...
                    if not os.path.exists(save_path):
                        os.makedirs(save_path, exist_ok=True)
                    torch.save(self.optimizer.importance[increment*4+l], save_path + str(l) + '.pt')
                ...
                for l in range(increment):
                    ...
                    if not os.path.exists(save_path):
                        os.makedirs(save_path, exist_ok=True)
                    torch.save(self.optimizer.importance[increment*5+l], save_path + str(l) + '.pt')
                ...
                for l in range(increment):
                    ...
                    if not os.path.exists(save_path):
                        os.makedirs(save_path, exist_ok=True)
                    torch.save(self.optimizer.importance[increment*6+l], save_path + str(l) + '.pt')
            ...
            for batch in tqb:
                with torch.no_grad():
                    if self.training_args.predict_with_generate:
                        print("predict_with_generate is True!!!!!!!!!!!!!!")
